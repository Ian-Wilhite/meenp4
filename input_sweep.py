...
bounds = Bounds([14, 0.2, 250, 0.05, 100], [19, 0.7, 500, 0.12, 200])
...
num_steps = 3 #time order N^5
for c0 in np.linspace(bounds.lb[0], bounds.ub[0], num_steps):
    for c1 in np.linspace(bounds.lb[1], bounds.ub[1], num_steps):
        for c2 in np.linspace(bounds.lb[2], bounds.ub[2], num_steps):
            for c3 in np.linspace(bounds.lb[3], bounds.ub[3], num_steps):
                for c4 in np.linspace(bounds.lb[4], bounds.ub[4], num_steps):
                                
                    print(f'loop iter: {c0}-{c1}-{c2}-{c3}-{c4}')
                    ...
                    options = {'maxiter': 0, 
                                ...
                                'disp' : True}
                    method = 'trust-constr'
                    res = minimize(obj_f, x0, method=method, constraints=nonlinear_constraint, 
                    ...
                    #             'disp' : True}
                    # method = 'SLSQP'
                    # res = minimize(obj_f, x0, method=method, constraints=ineq_cons, bounds=bounds, 
                    ...
                    values = {"parachute diameter [m]":res.x[0], 
                            ...
                            "time_to_calculate": time_elapsed}
